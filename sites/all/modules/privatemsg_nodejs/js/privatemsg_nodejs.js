...
Drupal.Nodejs.callbacks.privatemsgNodejs = {
  callback: function (message) {
    var pmNodejs = Drupal.privatemsgNodejs,
        ...
        $clone,
        uid = Drupal.settings.privateMsgNodeJS.user.uid;
...
Drupal.Nodejs.callbacks.privatemsgNodejsAlert = {
  callback: function (message) {
    ...
    if (!($('.wrapper-mini-chat-' + message.thread_id).length) && (window.location.href.indexOf(threadPath) < 1)) {
      if (!$('body').find('.pm-nj-alerts-wrapper').length) {
        $('body').append('<div class="pm-nj-alerts-wrapper" />')
      }
      $messageAlert = $(message.markup);
      $('.pm-nj-alerts-wrapper').prepend($messageAlert);
...
Drupal.Nodejs.callbacks.privatemsgNodejsStatus = {
  callback: function (message) {
    ...
    if (settings.user.uid !== message.uid && ($miniChat.length || $threadWrap.length)) {
...
Drupal.privatemsgNodejs = {
  ...
  prepareMarkup: function (content) {
    ...
    if (curentUser.uid !== messageAuthor.uid) {
      // Create DOM Object for author name.
      $user = $('<a>', {
        'href': settings.basePath + 'user/' + messageAuthor.uid,
        'title': Drupal.t('View user profile.'),
        'text': messageAuthor.name,
        'class': 'username'
      });
      // Set username.
      $content
